...
const GH_MODEL_FALLBACKS = ["gpt-5-mini", "gpt-4.1-mini", "gpt-4o"];
...
const GEMINI_MODEL_FALLBACKS = ["gemini-2.5-flash-lite", "gemini-2.5-flash", "gemini-2.0-flash"];
const AI_API_KEY = Deno.env.get("AI_API_KEY") || "";
const AI_BASE_URL = Deno.env.get("AI_BASE_URL") || "";
const AI_MODEL = Deno.env.get("AI_MODEL") || "";
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY);

const AI_TIMEOUT_MS = parseInt(Deno.env.get("AI_TIMEOUT_MS") || "30000");
const PROVIDER_PRIORITY: string[] = ["openrouter", "deepseek", "github", "google", "custom"];
...
async function callProvider(name: string, messages: { role: string; content: string }[], temperature: number, maxTokens: number, modelOverride?: string, jsonMode = false): Promise<string> {
  const headers: Record<string, string> = { "Content-Type": "application/json" };
  const body: Record<string, unknown> = { messages, temperature, max_tokens: maxTokens };
  let url = "";
  switch (name) {
    case "openrouter":
      if (!OPENROUTER_API_KEY) throw new Error("OpenRouter not configured");
      headers["Authorization"] = "Bearer " + OPENROUTER_API_KEY;
      headers["HTTP-Referer"] = "https://sansai-eight.vercel.app";
      headers["X-Title"] = "Sansi Sanskrit Tutor";
      body["model"] = modelOverride || OPENROUTER_MODEL;
    ...
    case "deepseek":
      ...
      body["model"] = modelOverride || DEEPSEEK_MODEL;
      if (jsonMode) body["response_format"] = { type: "json_object" };
      url = "https://api.deepseek.com/v1/chat/completions";
      break;
    case "github":
      if (!GITHUB_TOKEN) throw new Error("GitHub not configured");
      {
        headers["Authorization"] = "Bearer " + GITHUB_TOKEN;
        const primary = modelOverride || GH_MODEL;
        const candidates = [primary, ...GH_MODEL_FALLBACKS.filter((m) => m !== primary)];
        for (const model of candidates) {
          try {
            const ai = await fetchWithTimeout("https://models.inference.ai.azure.com/chat/completions", { method: "POST", headers, body: JSON.stringify({ ...body, model }) }, AI_TIMEOUT_MS);
            if (ai.ok) {
        ...
        throw new Error("GitHub models unavailable (tried: " + candidates.join(", ") + ")");
      }
    case "google":
      if (!GEMINI_API_KEY) throw new Error("Google not configured");
      {
        const primary = modelOverride || GEMINI_MODEL;
        const candidates = [primary, ...GEMINI_MODEL_FALLBACKS.filter((m) => m !== primary)];
        ...
        for (const model of candidates) {
          try {
            const ai = await fetchWithTimeout(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${GEMINI_API_KEY}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, AI_TIMEOUT_MS);
            if (!ai.ok) continue;
            const res = await ai.json();
            const content = res?.candidates?.[0]?.content?.parts?.map((p: any) => p.text || "").join("") || "";
            if (content.trim()) return content.trim();
          } catch { /* try next model */ }
        }
        throw new Error("Google models unavailable (tried: " + candidates.join(", ") + ")");
      }
    case "custom":
      if (!AI_API_KEY || !AI_BASE_URL) throw new Error("Custom AI not configured");
      headers["Authorization"] = "Bearer " + AI_API_KEY;
      body["model"] = modelOverride || AI_MODEL || "default";
      if (jsonMode) body["response_format"] = { type: "json_object" };
  ...
  const ai = await fetchWithTimeout(url, { method: "POST", headers, body: JSON.stringify(body) }, AI_TIMEOUT_MS);
  if (!ai.ok) throw new Error(name + " error: " + ai.status + " " + (await ai.text()).slice(0, 400));
  const res = await ai.json();
  return res?.choices?.[0]?.message?.content || "";
}

async function callAI(messages: { role: string; content: string }[], temperature: number, provider?: string, maxTokens = 1024, modelOverride?: string, jsonMode = false): Promise<string> {
  const requested = (provider || "").trim();
  const order = requested ? [requested, ...PROVIDER_PRIORITY.filter((p) => p !== requested)] : [...PROVIDER_PRIORITY];
  const errors: string[] = [];
  ...
  for (const name of order) {
    ...
    tried.add(name);
    try {
      const content = await callProvider(name, messages, temperature, maxTokens, modelOverride, jsonMode);
      if (content && content.trim()) return content.trim();
...
function extractJson(txt: string): string {
  ...
  if (s >= 0 && e > s) return txt.slice(s, e + 1).trim();
  return txt.trim();
}
  ...
  try {
    ...
    if (path.startsWith("auth/")) {
      ...
      if (sp === "login" && method === "POST") {
        ...
        const { data: sc } = await supabase.from("user_scores").select("*").eq("user_id", data.user.id).single();
        return json({ access_token: data.session.access_token, token_type: "bearer", user: { id: data.user.id, email: data.user.email, username: pr?.username || email.split("@")[0], display_name: pr?.display_name || "", role: pr?.role || "student", bio: pr?.bio || "", avatar_url: pr?.avatar_url || "", is_tutor: pr?.is_tutor || false, is_mentor: pr?.is_mentor || false, score_points: sc?.total_points || pr?.score_points || 0 } });
      }
      if (sp === "me" && method === "GET") {
        const uid = await requireUser(req);
        const pr = await getUser(uid);
        const { data: sc } = await supabase.from("user_scores").select("*").eq("user_id", uid).single();
        return json({ id: uid, username: pr?.username || "", display_name: pr?.display_name || "", role: pr?.role || "student", bio: pr?.bio || "", avatar_url: pr?.avatar_url || "", is_tutor: pr?.is_tutor || false, is_mentor: pr?.is_mentor || false, score_points: sc?.total_points || pr?.score_points || 0, created_at: pr?.created_at || "" });
      }
    }
    ...
    if (path.startsWith("learning/")) {
      ...
      if (sp === "tests" && method === "GET") {
        const { data } = await supabase.from("practice_tests").select("id,title,course_id,time_limit_minutes,passing_score,created_at,questions").order("created_at", { ascending: false });
        return json((data || []).map((t: any) => ({ id: t.id, title: t.title, course_id: t.course_id, time_limit_minutes: t.time_limit_minutes, passing_score: t.passing_score, created_at: t.created_at })));
      }
      ...
      const gen = sp.match(/^lessons\/([^\/]+)\/generate$/);
      if (gen && method === "POST") {
        await requireRole(req, "contributor");
        let genBody: any = {};
        try { genBody = await parseBody(req); } catch (pe) { genBody = { parseError: (pe as Error).message }; }
        const { data: lesson } = await supabase.from("lessons").select("*").eq("id", gen[1]).single();
        if (!lesson) return err("Lesson not found", 404);
        const { data: course } = await supabase.from("courses").select("id,title,language,level,description").eq("id", lesson.course_id).single();
        if (!OPENROUTER_API_KEY && !GITHUB_TOKEN) return err("AI service not configured", 503);
        const lang = course?.language || "sa";
        const level = course?.level || "beginner";
        const langNote = lang === "sa"
          ? "The lesson is about Sanskrit. Write body text mostly in English, but ALWAYS include Devanagari script and IAST romanization for every Sanskrit word, verse, or example."
          : "The lesson is about Hindi. Write body text in Hindi (Devanagari) with short English glosses where helpful for beginners.";
        const base = `Course: ${course?.title || ""} (${course?.description || ""}, level: ${level})\nLesson title: ${lesson.title}\nLesson id: ${lesson.id}\n${langNote}\nReturn ONLY valid JSON, no markdown fences, properly escaped.`;

        if (genBody.part === "save") {
          const content = genBody.content;
          if (!content?.introduction || !Array.isArray(content?.sections) || content.sections.length === 0 || !content?.summary) return err("Incomplete content", 400);
          const { data, error: ge } = await supabase.from("lessons").update({ content: JSON.stringify(content), duration: lesson.duration || "45 min" }).eq("id", gen[1]).select().single();
          if (ge || !data) return err(ge?.message || "Failed to save", 500);
          return json({ lesson_id: gen[1], saved: true, sections: content.sections.length, chars: JSON.stringify(content).length });
        }

        try {
          let txt = "";
          if (genBody.part === "plan" || !genBody.part) {
            const sys = `You are a professor planning a complete, self-contained textbook chapter for a ${level} learner. ${langNote}
${base}
Return ONLY valid JSON: {"introduction":"250-350 word opening starting from absolute zero — assumes no prior knowledge, gives the big picture, why the topic matters","headings":["7 to 9 section headings, ordered from fundamentals to depth"]}
Each heading must be a complete, self-contained unit covering a distinct aspect.`;
            txt = await callAI([{ role: "system", content: sys }, { role: "user", content: "Create the chapter plan." }], 0.7, genBody.provider, 2500, genBody.model, true);
            const j = JSON.parse(extractJson(txt));
            if (!j.introduction || !Array.isArray(j.headings)) throw new Error("Bad plan");
            return json({ part: "plan", introduction: j.introduction, headings: j.headings, echo: genBody });
          }
          if (genBody.part === "section") {
            const idx = genBody.section_index ?? 0;
            const heading = genBody.heading || `Section ${idx + 1}`;
            const sys = `You are a professor writing a textbook chapter for a ${level} learner. ${langNote}
${base}
Write ONLY the content of one section of the chapter.
Section ${idx + 1}: "${heading}"
Return ONLY valid JSON: {"heading":"${heading}","body":"500-900 words of dense, accurate, teaching-quality prose: define every term before using it, give concrete examples, verses with translations, paradigms, worked examples. Progress from fundamentals to depth. Never leave a term unexplained. Do not skip steps. Escape newlines properly."}`;
            txt = await callAI([{ role: "system", content: sys }, { role: "user", content: `Write section ${idx + 1}: ${heading}` }], 0.7, genBody.provider, 3000, genBody.model, true);
            const j = JSON.parse(extractJson(txt));
            if (!j.body) throw new Error("Empty section");
            return json({ part: "section", section_index: idx, heading: heading, body: j.body });
          }
          if (genBody.part === "summary") {
            const sys = `You are a professor finishing a textbook chapter for a ${level} learner. ${langNote}
${base}
Write the chapter's concluding summary.
Return ONLY valid JSON: {"summary":"200-300 word recap that ties every section together, restates key terms, and points to what to study next."}`;
            txt = await callAI([{ role: "system", content: sys }, { role: "user", content: "Write the summary." }], 0.7, genBody.provider, 1500, genBody.model, true);
            const j = JSON.parse(extractJson(txt));
            if (!j.summary) throw new Error("Empty summary");
            return json({ part: "summary", summary: j.summary });
          }
          return err("Unknown part", 400);
        } catch (e) {
          const msg = (e as Error).message || "parse error";
          if (msg.includes("OpenRouter") || msg.includes("AI service") || msg.includes("DeepSeek")) return err(msg, 502);
          return err("Generation failed: " + msg, 502);
        }
      }
    }
    ...
    if (path === "tutor" || path.startsWith("tutor/")) {
      ...
      if (sp === "chat" && method === "POST") {
        ...
        if (OPENROUTER_API_KEY || GITHUB_TOKEN) {
          ...
          } catch (e) { /* fall through to offline tutor */ }
        }
        return json({ ...tibuildReply(body.message || "", lang, diff), provider: "offline" });
      }
      if (sp === "translate" && method === "POST") {
        ...
        try {
          ...
          const raw = (body.text || params.text || "").trim();
          const tokens = raw.split(/[\s,.;:!?]+/).filter(Boolean);
          const parts = tokens.map((w: string) => {
            const clean = w.replace(/[।॥,.?!]/g, "");
            if (!clean) return w;
            const v = VERBS[clean];
            if (v) return `${clean} → ${v.iast} (${v.meaning})`;
            const d = DICT[clean];
            if (d) return `${clean} → ${d.iast} (${d.meaning})`;
            return w;
          });
          return json({ translated_text: parts.join(" | "), word_count: tokens.length, source: src, target: tgt, provider: "offline" });
        }
      }
    }

    if (path.startsWith("dictionary")) {
      const sp = path.replace(/^dictionary\/?/, "");
      if (sp === "lookup" && method === "GET") {
        const word = params.word?.trim() || "";
        if (!word) return err("Missing 'word' query param", 400);
        const { data, error: de } = await supabase.from("dictionary").select("*").or(`word.ilike.${word},root.ilike.${word}`).limit(10);
        if (de) return err(de.message, 500);
        const rows: any[] = data || [];
        const exact = rows.find((r) => r.word === word) || rows.find((r) => r.word?.toLowerCase() === word.toLowerCase());
        const pick = exact || rows[0] || null;
        if (!pick) {
          const inMem = DICT[word] || VERBS[word];
          if (inMem) return json({ word, meanings: [inMem.meaning], root: VERBS[word]?.root, derivations: [], pos: inMem.pos });
          return json(null);
        }
        return json({ word: pick.word, meanings: pick.meanings || [], root: pick.root, derivations: pick.derivations || [], pos: pick.pos });
      }
      if (sp === "compound-split" && method === "POST") {
        const body = await parseBody(req);
        const compound = (body.compound || "").replace(/[।॥\s]/g, "");
        if (!compound) return err("Missing 'compound'", 400);
        const { data } = await supabase.from("dictionary").select("word").limit(500);
        const known = new Set<string>((data || []).map((d: any) => d.word));
        for (const k of Object.keys(DICT)) known.add(k);
        for (const k of Object.keys(VERBS)) known.add(k);
        const sorted = [...known].sort((a, b) => b.length - a.length);
        const components: string[] = [];
        let rest = compound;
        while (rest.length > 0) {
          const hit = sorted.find((w) => rest.startsWith(w));
          if (hit) { components.push(hit); rest = rest.slice(hit.length); }
          else { components.push(rest); break; }
        }
        const explanation = components.length > 1
          ? `"${compound}" breaks down into: ${components.join(" + ")}. The final element carries the main meaning; earlier elements modify it.`
          : `"${compound}" is a simple word (no clear compound split found).`;
        return json({ components, explanation });
      }
    }

    if (path === "assessment/generate" && method === "POST") {
      const body = await parseBody(req);
      const topic = (body.topic || "sandhi").toLowerCase();
      const count = Math.min(Math.max(parseInt(body.count || "5", 10) || 5, 1), 20);
      const lang = body.language || "sa";
      const pool: { prompt: string; options: string[]; correctIdx: number; explanation: string }[] = [];
      if (/sandhi|सन्धि/.test(topic)) {
        for (const r of SANDHI_RULES) {
          pool.push({ prompt: `What is the sandhi rule in "${r.example}"?`, options: [r.name, "No sandhi", "Vowel lengthening", "Compound formation"], correctIdx: 0, explanation: r.description });
        }
      } else if (/case|vibhakti|विभक्ति/.test(topic)) {
        const cases = ["Nominative", "Accusative", "Instrumental", "Dative", "Ablative", "Genitive", "Locative", "Vocative"];
        const ex = ["रामः", "रामम्", "रामेण", "रामाय", "रामात्", "रामस्य", "रामे", "हे राम"];
        for (let i = 0; i < cases.length; i++) {
          const opts = [...cases].sort(() => Math.random() - 0.5);
          const correctIdx = opts.indexOf(cases[i]);
          pool.push({ prompt: `Which case is "${ex[i]}" in?`, options: opts, correctIdx, explanation: `"${ex[i]}" is the ${cases[i]} singular of राम.` });
        }
      } else {
        for (const e of EXERCISES) pool.push({ prompt: e.question, options: e.options, correctIdx: e.answer, explanation: e.explanation });
      }
      const questions = [...pool].sort(() => Math.random() - 0.5).slice(0, count);
      return json({ questions, topic, language: lang });
    }

    if (path === "analytics/evaluate" && method === "POST") {
      const body = await parseBody(req);
      const module = body.module || "general";
      const score = Math.min(Math.max(parseFloat(body.score || "0"), 0), 100);
      const weak = Array.isArray(body.weak_areas) ? body.weak_areas : [];
      const recommendations: string[] = [];
      if (score < 50) {
        recommendations.push(`Revise the fundamentals of ${module} before attempting advanced exercises.`);
        recommendations.push("Complete 3 practice sets on this module and re-test.");
        if (weak.length) recommendations.push(`Focus on weak areas: ${weak.join(", ")}.`);
      } else if (score < 80) {
        recommendations.push(`Solid progress in ${module}. Reinforce with spaced-repetition reviews.`);
        if (weak.length) recommendations.push(`Target practice for: ${weak.join(", ")}.`);
      } else {
        recommendations.push(`Excellent mastery of ${module}. Move to the next module.`);
        recommendations.push("Try time-limited drills to build fluency.");
      }
      const modules = ["declensions", "sandhi", "verbs", "compounds", "syntax", "vocabulary"];
      const next = modules[(modules.indexOf(module) + 1) % modules.length];
      const uid = body.user_id || "";
      if (uid) {
        await supabase.from("user_progress").upsert({ user_id: uid, current_track: next, xp: Math.round(score) }).maybeSingle();
      }
      return json({ recommendations, nextModule: next, score, module });
    }

    if (path === "viva/session" && method === "POST") {
      const sessionId = crypto.randomUUID();
      const { data } = await supabase.from("dictionary").select("word,meanings").limit(100);
      const words: any[] = data || [];
      const qpool = words.length >= 6
        ? words.sort(() => Math.random() - 0.5).slice(0, 6)
        : [["रामः", "Rama"], ["गच्छति", "he goes"], ["वनम्", "forest"], ["अहम्", "I"], ["गुरुः", "teacher"], ["पुस्तकम्", "book"]];
      const questions = qpool.map((w: any, i: number) =>
        `Q${i + 1}: What is the meaning of "${w.word || w[0]}"? Use it in a sentence.`
      );
      return json({ sessionId, status: "ready", questions });
    }
    ...
    if (path === "leaderboard/contributors" && method === "GET") {
      ...
      const { data } = await supabase.from("user_scores").select("*").order("total_points", { ascending: false }).limit(limit);
      const ids = (data || []).map((s: any) => s.user_id);
      const { data: pr } = ids.length ? await supabase.from("profiles").select("id,username,display_name,avatar_url").in("id", ids) : { data: [] };
      const pmap: Record<string, any> = {};
      for (const p of pr || []) pmap[p.id] = p;
      return json((data || []).map((s: any, i: number) => ({ rank: i + 1, user_id: s.user_id, username: pmap[s.user_id]?.username, display_name: pmap[s.user_id]?.display_name, avatar_url: pmap[s.user_id]?.avatar_url, score: { total_points: s.total_points, texts_uploaded: s.texts_uploaded, annotations_made: s.annotations_made, lessons_completed: s.lessons_completed } })));
    }
